...
import tasksUpdateModel from '../../../models/updateTasksSchema'
...
const getAllAssignedTasks = async (req, res) => {
    ...
    await tasksUpdateModel.find(query)
        .then((data) => {
...
const unAssignTasks = async (req, res) => {
    try {
        const unassignedTasks = await productUpdateModel.updateMany({ _id: req.body.selected }, {
            $unset: {
                assignDate: '',
                assignToDealAyoId: '',
                assignToName: '',
