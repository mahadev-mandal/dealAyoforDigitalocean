...
const reqFieldsArrForDataEntry = ['title',  ];
...
export default function AddProducts({ collName }) {
    ...
    const handleVerify = async () => {
        if (file) {
            ...
            if (checkReqFields(dataKeysArr, collName == 'products' ? reqFieldsArrForDataEntry : reqFieldsArrForUpdate)) {
                ...
                alert('title, model, supplier, MRP & category are required fields')
                setLoading(false)
                setSaving(false)
            }
    ...
    const handleSave = async () => {
        ...
        setData([])
        if (update) {
            await axios.put(`${baseURL}/api/${collName}`, data)
                .then((res) => {
                    setSaving(false)
                    setMsg(res.data)
                    setErrMsg(null);
                    ...
                    <Toolbar>
                        ...
                        >
                            ...
                            <Stack spacing={1} direction="row" alignItems="center">
                                ...
                                <Switch color="warning" onChange={e => setUpdate(e.target.checked)} />
